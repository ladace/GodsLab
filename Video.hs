module Video where
    ...
    instance RenderProcedure Video where
        render (Video img) script = do
            clear [ ColorBuffer ]
            Lua.callproc script "draw"
            --drawImage img 0 0 (fromIntegral $ mImageWidth img) (fromIntegral $ mImageHeight img)
    ...
    initialize w h = do
        ...
        image <- loadImage "assets/samples/test.png"
        return $ Video image
